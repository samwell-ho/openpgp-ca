...
`openpgp_ca ca new some.org`

`openpgp_ca user add -e alice@some.org -n "Alice"`

export public PGP Certificate of OpenPGP CA admin:

`openpgp_ca ca export > ca1.pub`

### (2) OpenPGP CA instance 2 (setup CA and create a user)

`export OPENPGP_CA_DB=/tmp/openpgp-ca2.sqlite`

`openpgp_ca ca new other.org`

`openpgp_ca user add -e bob@other.org -n "Bob"`

export public PGP Certificate of OpenPGP CA admin:

`openpgp_ca ca export > ca2.pub`
...
`openpgp_ca bridge list > ca2.signed`

Export user keys

`openpgp_ca user export > ca1.users`
...
`openpgp_ca bridge list > ca1.signed`

Export user keys

`openpgp_ca user export > ca2.users`
...
`openpgp_ca user add -e carol@third.org -n "Carol"`
